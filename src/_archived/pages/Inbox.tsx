...
function priorityLabel(score: number): { label: string; color: string } | null {
  if (score >= 10) return { label: 'High Priority', color: 'text-red-600 bg-red-500/10' };
  if (score >= 5) return { label: 'Medium', color: 'text-amber-600 bg-amber-500/10' };
  return null;
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-4">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="relative flex-shrink-0">
                        ...
                        <div className={`absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full ${config.bg} border border-card flex items-center justify-center`}>
        ...
        {/* Right panel: comment detail — desktop only */}
            ...
            <div className="sticky top-24 p-5 rounded-xl bg-card border border-border space-y-4">
              ...
              {selectedComment.is_replied && selectedComment.reply_text && (
                <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                  <p className="text-xs font-semibold text-emerald-600 mb-1">Your reply</p>
                  <p className="text-sm text-foreground">{selectedComment.reply_text}</p>
                </div>
              )}
              ...
              <div className="space-y-2">
                <textarea
                  ...
                  rows={3}
                  className="w-full p-3 rounded-xl border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none text-foreground"
                />
                {replyError && (
                  <p className="text-xs text-red-600 bg-red-500/10 rounded-lg px-3 py-2">{replyError}</p>
                )}
                {replySuccess && (
                  <p className="text-xs text-emerald-600 bg-emerald-500/10 rounded-lg px-3 py-2">
      ...
      {/* Mobile bottom sheet — shows when a comment is selected on small screens */}
      {selectedComment && (
        <div className="lg:hidden fixed inset-0 z-50 flex flex-col justify-end">
          ...
          />
          {/* Sheet */}
          <div className="relative bg-card rounded-t-2xl border-t border-border max-h-[85vh] flex flex-col shadow-2xl">
            {/* Handle + header */}
            <div className="flex-shrink-0 px-4 pt-3 pb-2">
              <div className="w-10 h-1 rounded-full bg-border mx-auto mb-3" />
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-foreground">Reply to comment</span>
                <button
                  onClick={() => setSelectedComment(null)}
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-foreground text-sm mb-0.5">
                    {selectedComment.author_name || selectedComment.author_username || 'Unknown'}
                    {selectedComment.author_username && (
                      <span className="text-xs text-muted-foreground font-normal ml-1">@{selectedComment.author_username}</span>
                    )}
                  </p>
              ...
              {selectedComment.is_replied && selectedComment.reply_text && (
                <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
                  <p className="text-xs font-semibold text-emerald-600 mb-1">Your previous reply</p>
                  <p className="text-sm text-foreground">{selectedComment.reply_text}</p>
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <button
                  onClick={sendReply}
                  disabled={!replyText.trim() || sendingReply}
                  className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold bg-primary text-primary-foreground hover:bg-primary/90 transition-all disabled:opacity-40"
                >
