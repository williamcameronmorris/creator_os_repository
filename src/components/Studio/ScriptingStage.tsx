...
export function ScriptingStage({ workflowId, contentType, onComplete, onSkip }: ScriptingStageProps) {
  ...
  const [ideaReasoning, setIdeaReasoning] = useState('');
  const [hasVoice, setHasVoice] = useState(false);
  const autoGeneratedRef = useRef(false);
  // Topic waiting for the first automatic generation. It runs only once the
  // brand is loaded: the server refuses a request with no brand.
  ...
  const loadWorkflowData = async () => {
    setLoading(true);
    // '*' rather than a column list so a database without idea_hook /
    // idea_reasoning yet still returns the row.
    ...
    if (data) {
      ...
      const row = data as { idea_hook?: string | null; idea_reasoning?: string | null };
      setIdeaHook(row.idea_hook || '');
      setIdeaReasoning(row.idea_reasoning || '');
  ...
  useEffect(() => {
    if (!pendingAutoTopic || !activeBrand || autoGeneratedRef.current) return;
    autoGeneratedRef.current = true;
    setPendingAutoTopic('');
    generateScript(pendingAutoTopic);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pendingAutoTopic, activeBrand]);
  ...
  const generateScript = async (topicOverride?: string) => {
    ...
    setAiError(null);
    try {
      if (!activeBrand) throw new Error('Your brands are still loading. Try again in a moment.');
      const { data: { user } } = await supabase.auth.getUser();
      ...
      const { data: fnData, error: fnError } = await supabase.functions.invoke('generate-script', {
        body: {
          ...
          contentType,
          mode,
          // The hook and reasoning the creator chose the idea for, so the
          // script opens on that hook instead of the topic alone.
          ...(ideaHook ? { hook: ideaHook } : {}),
          ...(ideaReasoning ? { reasoning: ideaReasoning } : {}),
