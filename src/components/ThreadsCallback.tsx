 ...
 * ThreadsCallback
 *
 * Handles the OAuth redirect from Threads after the user grants permissions.
 * URL pattern: /auth/threads/callback?code=ABC123
 *
 * Flow:
  ...
  useEffect(() => {
    ...
    const handleExchange = async () => {
      try {
        ...
        setHandle(result.threadsHandle || '');
        setStatus('success');
        setMessage(
          result.threadsHandle
            ? `@${result.threadsHandle} connected to Threads successfully!`
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-2xl p-8 text-center shadow-lg">
        ...
        <div className="flex items-center justify-center mb-6">
          <div className="w-16 h-16 rounded-2xl bg-black flex items-center justify-center shadow-md">
            <svg viewBox="0 0 192 192" fill="white" className="w-9 h-9">
        ...
        <h2 className="text-xl font-bold text-foreground mb-2">
          {status === 'loading' && 'Connecting Threads Account'}
        ...
        {status === 'success' && handle && (
          <div className="mt-4 p-3 rounded-xl bg-secondary border border-border">
            ...
            <p className="text-xs text-muted-foreground">Threads profile connected</p>
          </div>
        )}
