...
export function CreationStage({ workflowId, contentType, onComplete, onSkip }: CreationStageProps) {
  ...
  const loadWorkflowData = async () => {
    const { data } = await supabase
      ...
      .select('creation_notes')
      .eq('id', workflowId)
      .maybeSingle();

    if (data?.creation_notes) {
      const notes = data.creation_notes as any;
      if (notes.media_url) {
        setMediaUrl(notes.media_url);
      }
      if (notes.checklist_completed) {
        setChecklist(prev =>
          prev.map(item =>
            notes.checklist_completed.includes(item.id) ? { ...item, checked: true } : item
          )
        );
      }
    }
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await supabase
      ...
      .update({
        creation_notes: {
          media_url: publicUrl,
          checklist_completed: checklist.filter(i => i.checked).map(i => i.id)
        }
      })
      .eq('id', workflowId);
  };
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {checklist.map(item => (
              <label
                key={item.id}
                className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 hover:border-slate-200 hover:bg-slate-50 transition-all cursor-pointer"
              >
                <input
                  type="checkbox"
