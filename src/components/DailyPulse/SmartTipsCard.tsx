...
const tipStyles: Record<TipType, { iconBg: string; iconColor: string }> = {
  optimization: { iconBg: 'bg-emerald-100 dark:bg-emerald-900/30', iconColor: 'text-emerald-600 dark:text-emerald-400' },
  warning: { iconBg: 'bg-amber-100 dark:bg-amber-900/30', iconColor: 'text-amber-600 dark:text-amber-400' },
  opportunity: { iconBg: 'bg-blue-100 dark:bg-blue-900/30', iconColor: 'text-blue-600 dark:text-blue-400' },
};
  ...
  const renderHighlightedText = (text: string, highlight?: string) => {
    ...
    return parts.map((part, index) =>
      part.toLowerCase() === highlight.toLowerCase() ? (
        <span key={index} className="font-bold text-gray-900 dark:text-white">{part}</span>
      ) : (
        part
      )
    );
      ...
      <div className="space-y-4">
        {data.allCaughtUp ? (
          <div className="text-center py-8">
            ...
            <p className="text-lg font-semibold text-gray-900 dark:text-white mb-1">All caught up!</p>
            <p className="text-sm text-gray-500 dark:text-gray-400">You've reviewed all your insights for today</p>
          </div>
        ) : data.tips.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">No new insights yet</p>
            <p className="text-gray-400 dark:text-gray-500 text-xs mt-1">Post more content to get AI-powered tips</p>
          </div>
        ) : (
          data.tips.map((tip) => {
            ...
            return (
              <div key={tip.id} className="p-4 bg-gray-100 dark:bg-gray-800 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {tip.actionLabel && (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-medium hover:bg-gray-800 dark:hover:bg-gray-100 transition-colors"
                      >
                        {tip.actionLabel}
